jQuery(function ($) {
    $("form").bind("orchard-admin-pickimage-open", function (ev, data) {
        ...
        var url = location.href.substr(0, adminIndex)
            + "/Admin/FileManager/MediaPicker?uploadpath=" + (data.uploadMediaPath || "")
            + "&callback=" + callbackName;
        var w = window.open(url, "_blank", data.windowFeatures || "width=685,height=540,status=no,toolbar=no,menubar=no,resizable=no");
    });
});
